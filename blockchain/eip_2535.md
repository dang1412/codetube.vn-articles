...
- Các facet dùng chung 1 `struct` chứa tất cả các biến dùng trong các facet đó, sử dụng slot mặc định 0. Ngoài ra facet không khai báo thêm biến riêng nào khác.
...
Đối với các hàm không thay đổi không nhất thiết phải thay thế trỏ đến địa chỉ contract mới các bạn có thể thêm tham số chỉ định những hàm nào muốn update. Tuy nhiên để cho dễ quản lý (các function trong 1 contract code trỏ đến 1 địa chỉ) và code đỡ phức tạp mình chọn cách update toàn bộ những function có trong facet muốn upgrade. Các hàm nào có khả năng update cao có thể nên tách ra 1 facet riêng để tiện update về sau tuỳ cách thiết kế.
...
- Chuyển tất cả state variable vào struct `AppStorage`, update trong code tất cả các vị trí sử dụng biến này.
- Xoá bỏ constructor, chuyển code phần này vào trong hàm `init` của `DiamondInit` contract.
